...
group_size[graphname] = {
    attribute: np.zeros((NUM_RUNS, len(np.unique([G.nodes[v][attribute]]))))
    ...
    for run in range(NUM_RUNS):
        ...
        if succession:
            for vidx, val in enumerate(values):
                ...
                S_succession, opj_succession[val] = greedy(
                    list(h.nodes()),
                    math.ceil(len(node_attributes[val]) * K_SEEDS / len(G)),
                    oracle_values,
                )
        ...
        for vidx, value in enumerate(values):
            node_attributes[value] = [
                v for v in G.nodes() if G.nodes[v][attribute] == value
            ]
            f_multi_attribute[value] = valoracle_to_single(oracle_values, vidx)
            f_attribute[value] = multi_to_set(
                f_multi_attribute[value], n=N, g_nodes=G.nodes()
            )
        S_attribute = {}
        opt_attribute = {}
        if not succession:
            for value in values:
                S_attribute[value], opt_attribute[value] = (
                    greedy(
                        list(range(len(G))),
        ...
        partition = leidenalg.find_partition(
            graph_ig, leidenalg.ModularityVertexPartition
        )
        ...
        for community in community_all_label:
            communties[community] = [
                i for i, x in enumerate(partition.membership) if x == community
            ]
        ...
        for community in list(communties.keys()):
            ...
            community_label.append(
                list(
                    set(
                        [
                            G.nodes[ngIndex[int(cc)]][attribute]
                            for cc in communties[community]
                        ]
                    )
                )
        ...
        for i in range(MAX_GEN):
            ...
            if i % 10 == 0:
                print(f"Generation {i}: Best fitness: {Eval(p[0])}")

            new_p = [p[0]]

            while len(new_p) < POP_SIZE:
                p1, p2 = random.choices(p[:POP_SIZE // 2], 2)

                if random.random() < P_CROSSOVER:
                    cutted = random.randint(1, K_SEEDS - 1)
                    child = list(set(p1[:cutted] + p2[cutted:]))

                    while len(child) > K_SEEDS:
                        for node in parent1:
                            if node not in child:
                                child.append(node)
                                if len(child) == K_SEEDS: break
                else:
